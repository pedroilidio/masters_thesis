...
\begin{equation*}
    ...
    %= \lim_{r \to 1} \frac{- 2 [1 - \frac{1}{p}C(r)] c(r)}{- p}
...
%
The parameter $\sigma \in [0, 1]$, called supervision ratio,  % TODO consider alternative names
...
However, we must recall that the prototype value of each node is only dependent on its partition of the label matrix. Therefore, if $Y_\text{node}$ is close to being homogeneous, a new split is less likely to cause a big change in the outputs for the instances involved. Specifically, the most drastic prototype change possible occurs if we perfectly separate the instances with positive labels from the instances with negative labels.
...
We also speculate ... %XXX random, tree diversity

%To illustrate this idea consider performing the training process in two steps: unsupervised and supervised.

%and to investigate the impact of different $\sigma$ heuristics on the performance of semi-supervised decision trees, we propose the following heuristics for $\sigma$:

%The size of each node can also be an informative parameter.
