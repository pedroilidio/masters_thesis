...
An optimal binary classifier will thus present high TPR, TNR and precision while minimizing FPR.
...
To address this issue, it is commonly suggested the usage of precision-recall (PR) curves instead~\cite{ozenne2015precision,he2009learning}, where the precision is plotted as the vertical coordinate while the recall (or sensitivity, or TPR) is represented horizontally. In the PR space, considering fixed P and N, a simultaneous increase in TP and TN results in the following improvement in precision:
%
\begin{equation}
    \Delta Pr =
    \frac{TP + k}{TP + k + FP - k}
...
\subsection{Ideal descriptions of AUROC and AUPRC}
...
We can alternatively formulate this argument by expressing AUROC in terms of TN and TP themselves. While the variation of the decision values $ds$ is a considerably abstract concept in real-world scenarios, variations of TP and TN are directly related to the number of correctly classified instances of each class.
%
\begin{equation}
    A_\text{ROC}
...
%TODO always negative classifier
...
% In scenarios where reducing false positives is more important than increasing true negatives
In scenarios where constraining the number of false positives is as important as increasing the number of true positives, it is often recommended the usage of precision-recall curves as a replacement for ROC~\cite{ozenne2015precision,he2009learning,krawczyk2016learning,fernandez2018learning}.
%TODO the precision weights fp as blablabl
%
The area under the precision-recall curve behaves differently than AUROC under imbalanced circumstances.
%
\begin{multline}
    A_\text{PRC}
...
\end{multline}
%
In \autoref{eq:auprc0} we see that AVERAGE PRECISIONFJ

Furhtermore, the true label cumulative distributions ($F_0$ and $F_1$) appear each weighted by their respective class prevalences ($p$ and $n$). As a result, expressing the AUPR in terms of TP and FP
%When weighted in such a manner, a change in TP or TN affects the AUPRC in the same way since $dTP = - p f_1$ and $dTN = n f_0$.
...
% XXX define F and f = pf_1 + nf_0
% XXX interpret PR. average precision considering only a priori positive instances?
% TODO auroc becomes interesting again for PU

\subsection{AUROC as the normalized mean percentile ranking}
...
\begin{multline*}
    A_\text{ROC}
        = \frac{1}{n} \int_{0}^{1} (1 - C(r)) \left(1 - p C'(r)\right) \;dr
    =\\
        = \frac{1}{n} \int_{0}^{1} (1 - C(r)) \;dr
        + \frac{p}{n} \int_{0}^{1} (C(r) - 1) C'(r) \;dr
    =\\
        = \frac{1}{n} - \frac{1}{n}\int_{0}^{1} C(r) \;dr
        + \frac{p}{n} \left[ \frac{(C(r)-1)^2}{2} \right]_0^1
    %=\\
        = \frac{1}{n} - \frac{1}{n}\int_{0}^{1} C(r) \;dr - \frac{p}{2n}
    =\\
        = \frac{1}{n} - \frac{1-n}{2n} - \frac{1}{n}\int_{0}^{1} C(r) \;dr
        = \frac{1}{n} - \frac{1}{2n} + \frac{1}{2} - \frac{1}{n}\int_{0}^{1} C(r) \;dr
    =\\
        =  \frac{1}{2} + \frac{1}{n}\left[\frac{1}{2} - \int_{0}^{1} C(r) \;dr\right]
\end{multline*}
% \begin{multline*}
%     A_\text{ROC}
%         = \frac{1}{n} \int_{0}^{1} (1 - C(r)) \left(1 - p C'(r)\right) \;dr
%     =\\
%     = \frac{1}{p}\int_0^1 (1 - C(r)) \;dr
%         - \int_0^1 (1 - C(r)) C'(r)\;dr
%     =\\
%     = \frac{1}{p} \left[ 1 + \int_0^1 C(r) \;dr \right]
%         - \int_{C(0)}^{C(1)} (1 - C(r)) \;dC
%     =\\
%         = \frac{1}{p} \left[ 1 + \int_0^1 C(r) \;dr \right]
%         - \left[ \frac{(1 - C(r))^2}{2} \right]_0^1
%     =\\
%     % \frac{1}{2} + \frac{1}{p} \left[1 - \int_0^1 \int c(r) \;dr^2 \right]  % TODO: dr^2 notation?
%     \frac{1}{2} + \frac{1}{p} \left[1 - \int_0^1 C(r) \;dr \right]  % TODO: dr^2 notation?
% \end{multline*}
%
...
\begin{multline}
    A_\text{ROC}
        =  \frac{1}{2} + \frac{1}{n}\left[\frac{1}{2} - \int_{0}^{1} (r)'C(r)\;dr\right]
        = \frac{1}{2}
        + \frac{1}{n}\left[
            \frac{1}{2}
            - \left[rC(r)\right]_0^1
            +\int_{0}^{1} r\,c(r) \;dr
        \right]
        =\\
        = \frac{1}{2}
...
As can be deduced from \autoref{eq:matrix_factorization}, the dot product of each row of $U$ and $V$ approximates the corresponding element of $Y$:
%
\begin{equation}
...
\end{equation}
%
Under the assumption that the values in $U$ and $V$ follow zero-centered spherical gaussian distributions with variances $\frac{1}{\lambda}$ ($U\el{ij}\sim\mathcal{N}(0, \lambda_1^{-1}\mathbf{I})$ and $V\el{ij}\sim\mathcal{N}(0, \lambda_2^{-1}\mathbf{I})$), we recover the regularized objective function of \autoref{eq:lmf_regularization}~\cite{johnsonlogistic}.
%
\begin{multline}
...
As many previous authors~\cite{liu2017lpinrlmf,he2017simboost},%TODO cite more
...
\end{equation}
%
Expanding on the same logic, we can merge both supervised and unsupervised impurities to create a hybrid criterion function, that takes into account both the similarities between features and between labels to build a semisupervised decision tree. \autoref{eq:I_semi} defines such an impurity function. The parameter $\sigma \in [0, 1]$, called supervision ratio,% TODO consider alternative names
...
\begin{equation}
    ...
    %I_\text{semi-supervised}(X, Y) = (1 - \sigma) I_\text{unsupervised}(X) + \sigma I_\text{supervised}(Y)
    \label{eq:I_semi}
\end{equation}
...
% TODO list and describe our heuristics for sigma
% XXX

% trees naturally cluster
