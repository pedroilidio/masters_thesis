...
\begin{function}[tb]
    ...
    \BlankLine
    Draw $\tilde m$ columns (features) of $X$ without replacement\;
    \BlankLine
    
    \ForEach {feature index $f$ of the $\tilde m$ drawn features}{
...
\begin{function}[tb]
    ...
    $\Delta I^*$, $f^*$, $t^*\gets$ \FindSplit($X$, $Y$, $\tilde m$)\;
    \BlankLine
    ...
    \caption{BuildTree($X$, $Y$): Recursively build a Decision Tree}%, $\tilde m$)}
    \label{alg:buildtree}
\end{function}}
...
\While{Node is not a leaf}{
    \eIf{$\x_\text{new}\el{\text{Node.feature}}>$ Node.threshold}{
...
\caption{Predict(RootNode, $\x_\text{new}$): Compute a Decision Tree's prediction.}
\label{alg:predict}
\end{function}}


\newcommand{\algBipartiteTreePredict}{
    \begin{function}[tb]
    \KwIn{A new dyad and the root node of a Bipartite Decision Tree.}
    \KwOut{The tree's predicted value for the given feature vectors.}

    Node $\gets$ RootNode\;
    \BlankLine
    \While{Node is not a leaf}{
        %$\x_\text{new} \gets \x_\text{1, new}\text{ if Node.axis}=1$ else $\x_\text{2, new}$\;
        \lIf{$\text{Node.axis}=1$}{$\x_\text{new} \gets \x_\text{1, new}$}
        \lElse{$\x_\text{new} \gets \x_\text{2, new}$}
        \BlankLine
        \lIf{$\x_\text{new}\el{\text{Node.feature}}>$ Node.threshold}{
            Node $\gets$ Node.childRight
        }
        \lElse{Node $\gets$ Node.childLeft}
    }
    \BlankLine
    \Return \prototype($\x_\text{1, new}$, $\x_\text{2, new}$, Node.$Y$)\label{ln:prototype bipartite}
    \caption{BipartiteTreePredict(RootNode, $\x_\text{1, new}$, $\x_\text{2, new}$)} %: Compute a Decision Tree's prediction.}
    \label{alg:bipartite tree predict}
\end{function}
}
...
\begin{function}[tb]
    ...
    \BlankLine
    \eIf{\adapter is BGSO (\autoref{sec:bgso_trees})}{
...
\begin{function*}[tb]
    ...
    $Y_\text{new rows} \gets$ \predict{\primaryRows, $X_\text{1, new}$}
    
    \BlankLine
    \BlankLine
    %\tcp{Use the primary columns estimator to predict new label columns from $X_2$}
    $Y_\text{new cols} \gets$ \predict{\primaryColumns, $X_\text{2, new}$}
    \BlankLine
    \BlankLine
    ...
    \BlankLine
    \BlankLine
    %\tcp{Train the secondary columns estimator on $X_\text{2, new}$ and $Y_\text{new rows}^\intercal$}
    \train{\secondaryRows, $X_1$, $Y_\text{new cols}^\intercal$}\;
    \train{\secondaryColumns, $X_2$, $Y_\text{new rows}^\intercal$}\;
    \BlankLine
    \BlankLine
    $Y_\text{pred rows}\gets$ \predict{\secondaryRows, $X_\text{1, new}$}\;
    $Y_\text{pred cols}\gets$ \predict{\secondaryColumns, $X_\text{2, new}$}\;
    %\tcp{Train the secondary rows estimator on $X_\text{1, new}$ and $Y_\text{new cols}$}
    
    \BlankLine
    \BlankLine
    \If{Secondary estimators consider label dependencies}{
        \tcp{Skip predictions not referring to $X_\text{1, new}$ and $X_\text{2, new}$}
        $Y_\text{pred rows}\gets Y_\text{pred rows}\el{\cdot \; (j\; >\; n_1)}$\;
        $Y_\text{pred cols}\gets Y_\text{pred cols}\el{\cdot \; (j\; >\; n_2)}$\;
    }
